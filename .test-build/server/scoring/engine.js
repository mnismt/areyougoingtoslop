...
const AI_TOOL_PATTERN = '(?:github\\s+copilot|copilot|chatgpt|openai|claude|cursor|devin)';
const STRONG_AI_ATTRIBUTION_PATTERNS = [
    /\b(?:ai|llm)[-\s]?generated\b/i,
    new RegExp(`\\bgenerated\\s+(?:by|with|via|using)\\s+${AI_TOOL_PATTERN}\\b`, 'i'),
    new RegExp(`\\b(?:written|created|authored|drafted|produced|built)\\s+(?:by|with|via|using)\\s+${AI_TOOL_PATTERN}\\b`, 'i'),
...
const MEDIUM_AI_ATTRIBUTION_PATTERNS = [
    ...
    new RegExp(`\\b(?:assisted|paired)\\s+(?:by|with)\\s+${AI_TOOL_PATTERN}\\b`, 'i'),
    new RegExp(`\\b${AI_TOOL_PATTERN}\\s+(?:assisted|generated|draft(?:ed)?)\\b`, 'i'),
    /\b(?:ai|llm)[-\s]?assisted\b/i,
];
const TECHNICAL_CURSOR_CONTEXT_PATTERN = /\bcursor\s+(?:location|position|pos|column|row|line|screen|buffer|window|scroll|offset|move|moving|moved|stays?|state)\b/i;
...
const getAiAttributionStrength = (event) => {
    if (event.isMerge) {
        return 0;
    }
...
const computeWeightedStrength = (matches, totalWeight) => {
    if (totalWeight === 0) {
        return 0;
    }
    const weightedStrength = matches.reduce((sum, match) => sum + match.weight * match.strength, 0);
...
const computeSlopScore = (events, config = config_1.DEFAULT_SCORING_CONFIG, now = new Date()) => {
    ...
    const analyzedCommits = weightedEvents
        .slice(0, MAX_ANALYZED_COMMITS)
        .map((item) => {
        const flags = [];
        if (aiKeywordShas.has(item.event.sha))
            flags.push('ai_keyword');
        if (promptCrumbShas.has(item.event.sha))
            flags.push('prompt_crumb');
        if (largeGenericShas.has(item.event.sha))
            flags.push('large_generic');
        if (churnShas.has(item.event.sha))

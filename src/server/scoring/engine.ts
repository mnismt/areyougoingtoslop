...
const getRecencyWeight = (occurredAt: string, now: Date, config: ScoringConfig) => {
...
export const computeSlopScore = (
  ...
  const statsEvents = weightedEvents.filter(
    (item) => item.event.additions !== undefined || item.event.deletions !== undefined,
  );
  const statsCoverage =
  ...
  statsEvents.forEach((item) => {
    const changes =
      (item.event.additions ?? 0) + (item.event.deletions ?? 0);
    if (changes === 0) {
      return;
    }

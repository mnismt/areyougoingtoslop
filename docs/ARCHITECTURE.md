...
| **Apathy Ratio** | 15% | Among large commits (≥250 lines), ratio with generic messages (`fix`, `update`, `wip`, `cleanup`, `chore`, `tweak`, `refactor`, or ≤6 chars). Conventional initial commit messages (`init`, `initial`, `initial commit`, `first commit`, `bootstrap`, `scaffold`, `initialize`, `initialise`, `project init`, `repo init`, `initial version`) are explicitly excluded and never treated as generic. |
